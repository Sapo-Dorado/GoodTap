...
```bash
direnv allow                      # activate the Nix devShell (installs Elixir, Erlang, PostgreSQL, Node, etc.)
pg_ctl start -l .postgres/log     # start the local PostgreSQL instance
...
1. Replace `hardware/hetzner-hardware-configuration.nix` with the output and update the import in `flake.nix`
...
1. Start nginx and obtain a Let's Encrypt TLS certificate
1. Run database migrations
1. Start the GoodTap service
