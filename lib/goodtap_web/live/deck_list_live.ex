defmodule GoodtapWeb.DeckListLive do
  ...
  def handle_event("show_import", _params, socket) do
    {:noreply, assign(socket, show_import_modal: true, import_url: "", import_error: nil)}
  end
  ...
  def handle_event("import_deck", %{"url" => url}, socket) do
    user = socket.assigns.current_scope.user
    socket = assign(socket, importing: true, import_error: nil)

    case Decks.create_deck_from_url(user, String.trim(url)) do
      {:ok, deck} ->
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      >
        <div class="bg-gray-800 rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
          <h2 class="text-xl font-bold mb-4">Import Deck from Moxfield</h2>
